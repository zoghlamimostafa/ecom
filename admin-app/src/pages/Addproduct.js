...
let schema = yup.object().shape({
  ...
  subcategory: yup.string(), // Sous-catégorie optionnelle
...
const Addproduct = () => {
  ...
  // Filtrer les catégories principales et sous-catégories
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      const productPayload = {
        ...
        subcategory: values.subcategory || null, // Inclure la sous-catégorie
        tags: values.tags,
        color: color,
  ...
  const handleCategoryChange = (event) => {
    ...
    const categorySubcategories = subCategories.filter(sub => 
      sub.parentId && sub.parentId.toString() === categoryId
    );
    setAvailableSubcategories(categorySubcategories);
    
    // Réinitialiser la sous-catégorie si une nouvelle catégorie est sélectionnée
  ...
  useEffect(() => {
    ...
    // Initialiser la catégorie et sous-catégorie lors de l'édition
    if (isEdit && productData && productData.category) {
      setSelectedCategory(productData.category);
      const categorySubcategories = subCategories.filter(sub => 
        sub.parentId && sub.parentId.toString() === productData.category.toString()
      );
      setAvailableSubcategories(categorySubcategories);
    }
  }, [isEdit, productData, subCategories]);
