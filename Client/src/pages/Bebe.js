...
const Bebe = () => {
    ...
    // Ajout de l'état local pour éviter les appels multiples
    ...
    useEffect(() => {
        ...
        if (!isDataLoaded) {
            ...
            setIsDataLoaded(true);  // Marque que les données ont été chargées
        }
    }, [dispatch, isDataLoaded, authState]);
    ...
    // Filtrage des produits avec "tags === 'baby'" dans l'état des produits
    const infoProducts = Array.isArray(productState) ? productState.filter(item => item.tags === "baby") : []; // Admin
